...
main(int argc, char** argv)
{
	...
	fprintf(stderr,"repit using /dev/ser%1d at %d baud\n",
					portnum,baud);

	fprintf(stderr,"running repit for port %d\n",portnum);
	...
	for(int i = 0; i < 1000; i++)
	{
		loop = i % NUM;
		buf[loop] = 0;
		if((rlen = read(ser_fd,&buf[loop],1))==1)
			fprintf(stderr,"%d: rlen %d, read %s from %s\n", 
				i, rlen, prcstr(buf[loop]),portname);
		else
			fprintf(stderr,"read failed with rlen %d\n",rlen);

		if((wlen = write(ser_fd,&buf[loop],1))==1)
			fprintf(stderr,"%d: wlen %d, wrote %s to %s\n",
				i, wlen, prcstr(buf[loop]),portname);
		else
