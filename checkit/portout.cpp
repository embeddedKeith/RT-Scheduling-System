...
main(int argc, char** argv)
{
	...
	fprintf(stderr,"portout using /dev/ser%1d at %d baud\n",
					portnum,baud);

	fprintf(stderr,"running portout for port %d\n",portnum);
...
#undef ONLY_ASCII
	...
	{
		loop = i % NUM;
		wbuf[loop] = 'A'+(char)loop;
		if((wlen = write(ser_fd,&wbuf[loop],1))==1)
			fprintf(stderr,"%d: wlen %d, wrote %s to %s\n",
				i, wlen, prcstr(wbuf[loop]),portname);
		else
